...
class DemographicRecommender(BaseRecommender):
    ...
    def get_user_preferences(self, user_id):
        user_info = merge_userdata_ocupacion(self.data.datos_personales,self.data.ocupaciones)
        groups = asignar_grupos(user_info[user_info["user_id"] == user_id].iloc[0])
        return [(self.group_preferences[g],g) for g in groups]
